...
const BlogDetail = () => {
  ...
  const { data: blogdetail, isLoading } = useQuery("blogdetail", () =>
    fetch(`http://localhost:5000/blogdetail/${id}`).then((res) => res.json())
  );
  let ratings = [1, 2, 3, 4];
  if (isLoading) {
    return <Loader />;
    ...
    <div className="font-[Poppins] lg:w-full w-96">
      <div className="pb-10">
        ...
        <div className="my-4 lg:flex grid grid-cols-2 justify-center gap-4 text-center lg:mx-14">
          <h4 className="bg-primary rounded-sm text-white lg:px-4 px-2 lg:py-1 lg:text-lg text-base lg:w-20 w-14 ml-3">
            Hotel
          </h4>
          <div>
          ...
          <span className="lg:text-lg text-base">
            Author : Abu Jakaria
            <small className="lg:inline hidden"> ({role})</small>
          </span>
          <h4 className="lg:text-lg text-base">
            Ratings:
            {ratings.map((rating) => (
              <i
                key={Math.random() * 4}
        ...
        <div>
          <h1 className="lg:text-3xl text-2xl text-center my-7 font-semibold">
            {title}
          </h1>
