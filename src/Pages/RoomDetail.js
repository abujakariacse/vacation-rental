...
const RoomDetail = () => {
  ...
  const { data: roomDetail, isLoading } = useQuery("roomDetail", () =>
    fetch(`http://localhost:5000/room/${id}`).then((res) => res.json())
  );
  ...
  const [minCheckOutDate] = useState(
    new Date(today.setDate(today.getDate() - 30))
  );
  ...
  const { _id, name, image, rentFee, size, bed, view, max, detail } =
    roomDetail;
  return (
    <div className="bg-accent">
      <div>
        <img
          className="room-img w-80 lg:w-11/12 mx-auto rounded-md"
          src={image}
          alt=""
        />
      </div>
      <div className="grid lg:grid-cols-2 grid-cols-1 lg:mx-28 font-serif pb-5">
          ...
          <div className="card-body">
            ...
            <form onSubmit={handleSubmit} className="my-4">
              <input
                ...
                className="input w-full max-w-xs bg-accent placeholder-black  my-2"
                required
              />
        ...
        <div className="mx-7 lg:mx-0">
          <h2 className="text-secondary mt-4">
            <Link to="/room">Rooms and Suites</Link>{" "}
            <i className="fa-solid fa-angle-right text-xs"></i> <b>{name}</b>
          </h2>
          <h1 className="text-4xl text-secondary mt-6">{name}</h1>
          <h2 className="text-xl mb-6 mt-2">
            {" "}
            <span className="font-bold">
              <i className="fa-solid fa-tag"></i> ${rentFee}
            </span>{" "}
            (per night)
          </h2>
          <div>
            ...
            <div className="my-6">
              ...
              <p className="text-lg">
                ...
                <span className="font-bold">Holding capacity:</span> {max}
              </p>
            </div>
          </div>
          <div>
